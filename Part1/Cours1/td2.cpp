...
void init() {
    ...
    initialisation("lapin.obj", &nbSommets, &nbFacettes);

    
    GLfloat* coord = new GLfloat[3 * nbSommets];
    GLuint* indice = new GLuint[3 * nbFacettes];
    float* bbox = new float[3 * 8];

    GLfloat* coordBB = new GLfloat[6];

    lecture("lapin.obj", nbSommets, nbFacettes, coord, indice, bbox);

    boundingbox(bbox, coordBB);

    

    glGenBuffers(2, vboid);
    glBindBuffer(GL_ARRAY_BUFFER, vboid[0]);
    glBufferData(GL_ARRAY_BUFFER, 3 * nbSommets * sizeof(float), coord, GL_STATIC_DRAW);
    ...
    */
    ...
    glBufferData(GL_ELEMENT_ARRAY_BUFFER, 3 * nbFacettes * sizeof(GLuint), indice, GL_STATIC_DRAW);
    ...
    trans_initial = glm::translate(glm::mat4(1.0f), glm::vec3(-(bbox[5]-bbox[2]), -(bbox[4]-bbox[1]), -(bbox[3]-bbox[0])));
...
void Display(void) {
    ...
    glDrawElements(GL_TRIANGLES, 3 * nbFacettes, GL_UNSIGNED_INT, NULL);
